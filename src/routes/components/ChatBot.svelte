<script>
    ...
    import botIcon from "$lib/assets/chat-svgrepo-com (1).svg";
    ...
    const getAllChat = () => {
        try{
            return localStorage.getItem("allCeemaAgentChats") ? JSON.parse(localStorage.getItem("allCeemaAgentChats")) : "No chat available";
        }catch(err){
            console.log(err)
        }
    }
    ...
    const storeChat = (prompt, reply) => {
        ...
        const allChats = localStorage.getItem("allCeemaAgentChats") ? JSON.parse(localStorage.getItem("allCeemaAgentChats")) : localStorage.setItem("allCeemaAgentChats",JSON.stringify([chat]));

        localStorage.setItem("allCeemaAgentChats", JSON.stringify([...allChats, chat]));
...
<section class="{openChat ? 'translate-y-0' : 'translate-y-500'} tansition-transform duration-1000 transform flex fixed flex-col justify-start z-40 bottom-0 right-0 w-full h-full sm:w-[400px] sm:h-[400px] sm:drop-shadow-xl sm:rounded-lg sm:overflow-hidden sm:bottom-5 sm:right-5 bg-white">
    <div class="py-2 w-full bg-[#000] text-white text-2xl text-center flex items-center justify-between px-5 space-y-2">
        <div class="flex flex-col space-y-2 items-center justify-center">
            <h2><span class="text-[#e07b1c]">Ceema</span> Chatbot</h2>
    ...
    <form onsubmit={() => sendPrompt()} class="flex justify-between items-center gap-5 p-2 bg-[#000]">
        <input bind:this={prompt} type="text" autofocus placeholder="How can I help you..." class="flex-1 w-full bg-white rounded-2xl border-2 border-[#e07b1c]" />
        <button type="submit" class="bg-white px-3 py-2 rounded-2xl border-2 border-[#e07b1c] " >Send</button>
    </form>
</section>
...
<section class="{openChat ? 'opacity-0 hide' : 'show opacity-100'} transition-opacity duration-1000 fixed bottom-5 right-5 sm:bottom-10 sm:bottom-10 z-40 shadow-lg rounded-full bg-white">
    <div onclick={() => hundleOpenChat()} role="button" tabindex="0" onkeydown={() => openChat} class="w-[70px] sm:w-[80px] cursor-pointer">
        <img src={botIcon} alt="SVA Agent" class="w-full h-full object-cover icon" />
    </div>
</section>
